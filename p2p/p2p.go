...
// MaxBlocksize is the number of bytes a request can ask for
...
func (t *Torrent) startDownloadWorker(peer peers.Peer, workQueue chan *pieceWork, results chan *pieceResult) {
	...
	for pw := range workQueue {
		if !c.Bitfield.HasPiece(pw.index) {
			workQueue <- pw // Put piece back in the queue
			continue
		}
		...
		if err != nil {
			log.Printf("Piece number %d failed integrity check", pw.index)
			workQueue <- pw // Put piece back in the queue
			continue
		}
